(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    var output = []; //1 2
    var booleanCatch = []; //true false
    if (iterator) {
      var newArray = array.map(iterator);
        for (var i = 0; i < newArray.length; i++) {
          if (!booleanCatch.includes(newArray[i])) {
            booleanCatch.push(newArray[i]);
            output.push(array[i]);
          }
        }
    } else {
    ...
    if (Array.isArray(collection)) {
      if (accumulator === undefined) {
        ...
        for (var l = 0; l < collection.length; l++) {
          accumulator = iterator(accumulator, collection[l]);
        }
      } 
    } else {
      ...
      if (accumulator === undefined) {
        ...
        for (var l = 0; l < values.length; l++) {
          accumulator = iterator(accumulator, values[l]);
        }
      }
    }  
    return accumulator;
  ...
  _.every = function(collection, iterator) {
    ...
    return true
    // var truthy = _.map(collection, iterator); //true false
